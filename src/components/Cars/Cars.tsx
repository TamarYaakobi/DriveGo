...
//         : undefined
//       const range = await carService.getYearRange(categoryId)
//       setYearRange(range)
//     }
//     if (categories.length) fetchYearRange()
...
//     const filteredCars = await carService.getCarsByYearAndCategory(year, categoryId)
...
//       )}
//       {selectedYear && (
//         <p className="year-filter__current">מציג רכבים משנת {selectedYear} ומעלה</p>
//       )}
//       <div className="Cars">
//         {cars.map((i) =>
//           <CarCard
...
const getSliderBackground = (range: { min: number, max: number } | null, value: number | null) => {
  ...
  // חישוב אחוז פשוט ונקי משמאל לימין (מתואם ל-LTR המקומי)
  const percentage = total > 0 ? ((current - range.min) / total) * 100 : 0;

  // צובע בזהב את החלק הימני (הערכים הגבוהים שנבחרו) ומשאיר את השמאלי כהה
